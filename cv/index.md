...
| `aerovision_worker/` | Worker package with settings, logging, device, database, storage path, model runtime, startup, and queue modules. |
| `tests/` | Worker tests for startup, settings, logging, storage, database, model runtime, queue helpers, and PostgreSQL queue behavior. |
...
| `python -m aerovision_worker.main` | starts worker polling; claimed jobs run model preflight and then fail with a placeholder until processing is implemented |
